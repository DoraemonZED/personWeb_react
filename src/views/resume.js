...
export default function Resume(){
    useLayoutEffect(() => {
        //简历页面设置body全屏不可滚动，离开本页后释放
        let mbody = document.querySelector('body');
        mbody.style = 'background-color: #02162b;overflow: hidden;'
        return () => {
            mbody.style = ''
        }
    });
    ...
    //头部导航
    let navArr = ['HOME', 'TECHNOLOGY', 'HISTORY', 'PROTFOLIO', 'SERVERICES', 'CONTACT'];
    let [navBar, setnavBar] = useState({ width: 0, left: 0 });
    let [navIndex, setnavIndex] = useState(0);
    let ulRef = useRef();
    useEffect(() => {
        let liItem = ulRef.current.childNodes[navIndex];
        setnavBar({ width: liItem.clientWidth, left: liItem.offsetLeft })
    },[navIndex])
        ...
        if(index !== navIndex){
            switch (index) {
                case 0:
                    ...
                    break;
                case 1:
                    frameRef.current.scrollTop = 902
                    break;
                case 2:
                    frameRef.current.scrollTop = 1860
                    break;
                case 3:
                    frameRef.current.scrollTop = 2768
                    break;
                case 4:
                    frameRef.current.scrollTop = 3234
                    break;
                case 5:
                    frameRef.current.scrollTop = 3796
                    break;
                default:
                    break;
            }
    ...
    //屏幕滚动时
    let frameRef = useRef()
    let [frameTop, setframeTop] = useState(0)
    function frameScroll(){
        setframeTop(frameRef.current.scrollTop)
        
        if(900 > frameTop && frameTop >= 0){
            setnavIndex(0)
        }else if(1858 > frameTop && frameTop >= 900){
            setnavIndex(1)
        }else if(2766 > frameTop && frameTop >= 1858){
            setnavIndex(2)
        }else if(3232 > frameTop && frameTop >= 2766){
            setnavIndex(3)
        }else if(3794 > frameTop && frameTop >= 3232){
            setnavIndex(4)
        }else if(frameTop >= 3794){
            setnavIndex(5)
        }
                    ...
                    <div className="menu">
                        <div className="item">
                            <ul ref={ulRef}>
                                {
                                    navArr.map((item, index) => (
                                            <li key={ index } className={ index===navIndex?'active':null } onClick={() => {navClick(index)} }>
                                                { item }
                                            </li>
                                        )
                                    )
                            ...
                            <div className="navBar">
                                <span style={ navBar }></span>
                            </div>
                        </div>
                        <a href="/" className="mc_a_btn">返 回</a>
                    </div>
                </div>
            </div>
            <div className="banner" id="HOME" style={{ backgroundPositionY: - frameTop * 0.5  +50+ 'px' }}></div>
            <div className="container">
                <div className="main_box">
                    ...
                    <div className="content_me">
                        ...
                        <div className="last_bar_line"></div>
                        <div className="last_card mc_box_resume">
                            <span>@2022 BUILD WED SERVERS.</span><span>POWER BY:<span style={{ color:'#afb42b' }}> YANGWEI</span></span>
                        </div>
                    </div>
                </div>
            </div>
            <span className="scrollBar">
                <span className="scrollbar_thumb"></span>
            </span>
        </div>
    )
}